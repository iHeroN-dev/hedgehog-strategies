...
def initialize(
    ...
    if nameOverride == "":
        self.name = concat(DetailedERC20(token).symbol(), " hhVault")
    else:
        self.name = nameOverride
    if symbolOverride == "":
        self.symbol = concat("hhv", DetailedERC20(token).symbol())
    else:
    ...
    self.managementFee = 20  # 2% per year #HH no management fees
